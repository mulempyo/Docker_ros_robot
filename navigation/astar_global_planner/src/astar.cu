    ...
    for (int dx = -1; dx <= 1; ++dx) {
        for (int dy = -1; dy <= 1; ++dy) {
            ...
            if (nx >= clearance_cells && ny >= clearance_cells &&
                ...
                float th = atan2f((float)dy, (float)dx);
                unsigned char cost = costmap[ny * width + nx];
                if (cost <= 100) {
    ...
    int* open_list_size)
{
    ...
    for (int j = 0; j < count; ++j) {
        ...
        double tentative_g = g_cost[current_index] + hypot((double)(neighbor_x - current_x), (double)(neighbor_y - current_y));

        if (tentative_g < g_cost[neighbor_index]) {
            g_cost[neighbor_index] = tentative_g;

            double f_cost = tentative_g + (abs(neighbor_x - neighbor_y) + abs(static_cast<int>(goal_x) - static_cast<int>(goal_y)));
