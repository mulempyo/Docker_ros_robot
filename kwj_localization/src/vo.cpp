...
public:
    VisualOdometryWithIMU(ros::NodeHandle& nh) : it_(nh), first_frame_(true), x_(0.0), y_(0.0), yaw_(0.0),
        ...
        orb_ = cv::ORB::create(
                2000,       // nfeatures
                1.1f,       // scaleFactor (더 작게 줄수록 더 정밀하게 탐색)
                8,          // nlevels
                31,         // edgeThreshold
                0,          // firstLevel
                2,          // WTA_K
                cv::ORB::HARRIS_SCORE,
                31,         // patchSize
                20          // fastThreshold (낮출수록 더 많이 검출됨)
            );
    }
...
private:
   ...
   float getDepthFromPointCloud(const sensor_msgs::PointCloud2ConstPtr& cloud_msg, int x, int y) {
    ...
    void imageProcessing(const cv::Mat& img, const ros::Time& stamp) {
        ...
        cv::Ptr<cv::BFMatcher> matcher_ = cv::BFMatcher::create(cv::NORM_HAMMING, true);
        ...
        int white_pixels = cv::countNonZero(motion_mask);
        if (white_pixels < 1000) {
            ROS_WARN("Motion mask too restrictive. Disabling mask for this frame.");
            orb_->detectAndCompute(img, cv::noArray(), curr_keypoints, curr_descriptors);
        } else {
        ...
        if (curr_descriptors.type() != CV_8U || previous_descriptors_.type() != CV_8U) {
            ROS_ERROR("Descriptors must be CV_8U for ORB. Got types: %d and %d", previous_descriptors_.type(), curr_descriptors.type());
            return;
        }

        if (previous_descriptors_.rows < 2 || curr_descriptors.rows < 2) {
            ROS_WARN("Too few descriptors: prev=%d, curr=%d", previous_descriptors_.rows, curr_descriptors.rows);
            return;
        }

        ROS_WARN("Descriptor types: previous=%d, current=%d", previous_descriptors_.type(), curr_descriptors.type());
        ROS_WARN("Descriptor types: previous=%d, current=%d", previous_descriptors_.type(), curr_descriptors.type());
        ROS_WARN("Descriptor cols: previous=%d, current=%d", previous_descriptors_.cols, curr_descriptors.cols);
        ROS_WARN("Descriptor rows: previous=%d, current=%d", previous_descriptors_.rows, curr_descriptors.rows);
        ...
        for (const auto& match : raw_matches) {
            ...
            if (!std::isfinite(d) || d <= 0.01 || d > 10.0f) {
                continue;  
            }
        ...
        if (pts3d.size() < 4 || pts2d_float.size() < 4 || pts3d.size() != pts2d_float.size()) {
            ROS_WARN("solvePnPRansac: invalid input size (3D=%lu, 2D=%lu)", pts3d.size(), pts2d_float.size());
            return;
        }
